...
class GiveawayListeners(commands.Cog):
    ...
    async def on_voice_state_update(
        ...
        if (before.channel is not None and not after.channel is not None) or (
            isinstance(before.channel, disnake.StageChannel)
            and not isinstance(after.channel, disnake.StageChannel)
        ):
            ...
            for entry in entries:
                ...
                if (
                    ...
                    and not after.channel is not None
                ):
                    await self.db.delete_user_giveaway_entries(entry[1], member.id)
                elif (
                    giveaway[0][9] == "Tribune"
                    and isinstance(before.channel, disnake.StageChannel)
                    and not isinstance(after.channel, disnake.StageChannel)
                ):
                    await self.db.delete_user_giveaway_entries(entry[1], member.id)
    ...
    async def on_button_click(self, interaction: disnake.MessageInteraction):
        if interaction.component.custom_id == "giveaway_join":
            ...
            if not message or message[0][8] < datetime.datetime.now():
                embed = disnake.Embed(
                    description="The giveaway has already ended!", color=0x2F3136
                )
                await interaction.response.send_message(embed=embed, ephemeral=True)
                return
            if message[0][9] == "Tribune":
                if not await GiveawayFunctions(self.bot).check_if_user_on_tribune(
                    interaction
                ):
                    embed = disnake.Embed(
                        description="You must be on a tribune to join the giveaway!",
                        color=0x2F3136,
                    )
                    await interaction.response.send_message(embed=embed, ephemeral=True)
                    return
            elif message[0][9] == "Voice":
                if not await GiveawayFunctions(self.bot).check_if_user_in_voice_channel(
                    interaction
                ):
                    embed = disnake.Embed(
                        description="You must be in a voice channel to join the giveaway!",
                        color=0x2F3136,
                    )
                    await interaction.response.send_message(embed=embed, ephemeral=True)
                    return
            entry = await self.db.get_user_giveaway_entry(
                interaction.message.id, interaction.author.id
            )
            if entry:
                embed = disnake.Embed(
                    description=f"{
                        interaction.author.mention}, You ** left** the giveaway!",
                    color=0x2F3136,
                )
                await interaction.response.send_message(embed=embed, ephemeral=True)
                await self.db.delete_user_giveaway_entries(
                    interaction.message.id, interaction.author.id
                )
            else:
                await self.db.add_giveaway_entry(
                ...
                embed = disnake.Embed(
                    description=f"{
                        interaction.author.mention}, You **entered** the giveaway!",
                    color=0x2F3136,
                )
                await interaction.response.send_message(embed=embed, ephemeral=True)
            ...
            if not entries:
                embed = disnake.Embed(
                    description="No one entered the giveaway.", color=0x2F3136
                )
                await interaction.response.send_message(embed=embed, ephemeral=True)
            else:
                embeds = []
                for i in range(0, len(entries), 50):
                    entries_mentions = "\n".join(
                        [
                            f"{i+1}) <@{entry[2]}>"
                            for i, entry in enumerate(entries[i: i + 50])
                        ]
                    )
                    embed = disnake.Embed(
                        title=f"Participants of the giveaway, total {
                            len(entries)}",
                        description=entries_mentions,
                        color=0x2F3136,
                    )
                    embed.set_thumbnail(
                        url=interaction.author.avatar.url
                        if interaction.author.avatar
                        else interaction.author.default_avatar.url
                    )
                    embeds.append(embed)
                await interaction.response.send_message(
                    embed=embeds[0], view=EntriesPaginator(embeds), ephemeral=True
                )

        elif interaction.component.custom_id == "giveaway_reroll":
            if not interaction.author.guild_permissions.administrator:
                embed = disnake.Embed(
                    description="You must be an administrator to reroll the giveaway!",
                    color=0x2F3136,
                )
                return await interaction.response.send_message(
                    embed=embed, ephemeral=True
                )
            winners = await GiveawayFunctions(self.bot).choose_winners(
                interaction.message.id
            )
            ...
            await interaction.response.send_message(
                embed=disnake.Embed(
                    description="The giveaway has been rerolled!", color=0x2F3136
                ),
                ephemeral=True,
            )
