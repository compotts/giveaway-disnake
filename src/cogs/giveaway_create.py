...
class Giveaway(commands.Cog):
    ...
    async def giveaway_create(self, 
        ...
        await self.db.add_giveaway(message.id, interaction.guild.id,
                                   ...
                                   prize, winers, start_time, end_time, str(voice), "False")
        self.bot.loop.create_task(GiveawayFunctions(self.bot).start_giveaway(interaction.guild.id, message.id, end_time))
        await interaction.response.send_message(embed=disnake.Embed(description="You have successfully created a giveaway!", color=0x2F3136), ephemeral=True)