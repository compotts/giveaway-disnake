...
class Database:
    ...
    async def create_tables(self):
        ...
        CREATE TABLE IF NOT EXISTS `giveaways` (
            ...
            `ended` VARCHAR(255) NOT NULL DEFAULT 'False'
        );
    ...
    async def add_giveaway(self, message_id, guild_id, channel_id, host_id, prize, winers, start_time, end_time, voice, ended):
        query = """
        INSERT INTO `giveaways` (message_id, guild_id, channel_id, host_id, prize, winers, start_time, end_time, voice, ended)
        VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s);
        """
        await db_pool.execute_query(query, (message_id, guild_id, channel_id, host_id, prize, winers, start_time, end_time, voice, ended))
...
####
    ...
    async def set_true_ended(self, message_id):
        query = """
        UPDATE `giveaways` SET ended = 'True' WHERE message_id = %s;
        """
        await db_pool.execute_query(query, (message_id,))